...
\textbf{(1) Per-frame activations.} The probe averages each layer over time, giving one vector per clip --- far too few examples to fit a large SAE. So we re-run the same frozen encoders and keep the activations \emph{un-pooled}: one vector per audio frame, per layer (Whisper's padding frames masked). CREMA-D layer~13 alone gives $940{,}273$ frames.
